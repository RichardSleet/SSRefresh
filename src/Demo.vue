<template>
    <refresh :onCritical="onCritical" :onRefresh="onRefresh">
        <template slot="refresh-view">
            <div class="refresh-contienr">
                <img :src="img" ref="loadingIcon"/> 加载中
            </div>
        </template>
        <div class="show-content">
            <div v-for="(item, index) of list" class="list-item" :key="index">
                {{item.title}}
            </div>
        </div>
    </refresh>
...
<script>
/**
 * 注明: Demo 的测试列表的数据来自 https://github.com/watson-yan/vue-pullrefresh
 */
...
export default  {
    ...
    methods: {
        onCritical(type) {
            if (type === 1) {
                this.$refs.loadingIcon.style.transform = 'rotate(180deg)';
            } else {
                this.$refs.loadingIcon.style.transform = 'rotate(0deg)';
            }
        },
        onRefresh(onEndRefresh) {
            this.img = require('./assets/refresh.png');
            this.$refs.loadingIcon.style.animation = '1s rotateIcon linear infinite';
            setTimeout(() => {
                this.list = this.backup;
                onEndRefresh();
                setTimeout(() => {
                    this.img = require('./assets/arrow.png');
                    this.$refs.loadingIcon.style.transform = 'rotate(0deg)';
                    this.$refs.loadingIcon.style.animation = '';
                }, 500)
            }, 2000);
        }
    }
